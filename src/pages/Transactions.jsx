...
import { fetchTransactions, addTransaction, updateTransaction, deleteTransaction } from '@/lib/api'
...
const CATEGORIES = ['Income', 'Food', 'Transport', 'Shopping', 'Health', 'Utilities', 'Entertainment', 'Education']
...
export default function Transactions() {
  ...
  const totalIncome = filtered.filter(t => Number(t.amount) > 0).reduce((sum, t) => sum + Number(t.amount), 0)
  const totalExpenses = filtered.filter(t => Number(t.amount) < 0).reduce((sum, t) => sum + Math.abs(Number(t.amount)), 0)
  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = {
        ...
        amount: form.category === 'Income' ? Math.abs(Number(form.amount)) : -Math.abs(Number(form.amount)),
        date: form.date,
      }
      if (editingId) {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="dark:bg-slate-800 dark:border-slate-700 w-[90vw] max-w-md rounded-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Amount (PKR)</label>
              <Input
                type="number"
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-4">
        <Card className="dark:bg-slate-800 dark:border-slate-700">
          <CardContent className="pt-4 md:pt-6 px-3 md:px-6">
            <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400">Total Income</p>
            <p className="text-base md:text-xl font-bold text-green-600">PKR {totalIncome.toLocaleString()}</p>
          </CardContent>
        </Card>
        <Card className="dark:bg-slate-800 dark:border-slate-700">
          <CardContent className="pt-4 md:pt-6 px-3 md:px-6">
            <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400">Total Expenses</p>
            <p className="text-base md:text-xl font-bold text-red-500">PKR {totalExpenses.toLocaleString()}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="space-y-2">
              {filtered.map((t) => (
                <div key={t.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors border border-slate-100 dark:border-slate-700">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className={`w-2 h-2 rounded-full shrink-0 ${Number(t.amount) > 0 ? 'bg-green-500' : 'bg-red-500'}`} />
                  ...
                  <div className="flex items-center gap-1 md:gap-3 shrink-0 ml-2">
                    <Badge className={`text-xs hidden sm:inline-flex ${CATEGORY_COLORS[t.category] || 'bg-slate-100 text-slate-700'}`}>
                      {t.category}
                    </Badge>
                    <span className={`text-xs md:text-sm font-semibold ${Number(t.amount) > 0 ? 'text-green-600' : 'text-red-500'}`}>
                      {Number(t.amount) > 0 ? '+' : '-'}PKR {Math.abs(Number(t.amount)).toLocaleString()}
                    </span>
                    <Button variant="ghost" size="icon" className="text-slate-400 hover:text-blue-500 h-8 w-8" onClick={() => openEdit(t)}>
                      <Pencil size={14} />
                    </Button>
                    <Button variant="ghost" size="icon" className="text-slate-400 hover:text-red-500 h-8 w-8" onClick={() => handleDelete(t.id)}>
                      <Trash2 size={14} />
                    </Button>
                  </div>
                </div>
